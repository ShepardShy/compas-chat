<script setup lang="ts">
	...
	const imageTwo = computed(() => {
		const shownImagesLength = message.value.images?.slice(0, 4).length;
		return isMobileSize.value && shownImagesLength === 4 ? "span 1" : "1";
	});
...
<template>
	<FansyBox
		class="video-message"
		:class="{
			'video-message_mobile': isMobileSize,
		}"
	>
		<div
			class="video-message__images"
			:style="{
		...
		>
			<div
				...
				:class="{
					'video-message__hide-images_last': isImagesMoreThenShown && idx === 3,
				}"
				:style="{
			...
			>
				<div
					...
					class="video-message__hide-images-length"
				>
					<video
				...
				<div
					v-if="!(isImagesMoreThenShown && idx === 3)"
					class="video-message__info"
				>
					<div class="video-message__time">
		...
		>
			...
			<div
				v-if="message.comment"
				class="video-message__comment-info"
			>
				<div class="video-message__comment-data">
					{{ messageTime() }}
				</div>
				<ViewedMessageIcon
