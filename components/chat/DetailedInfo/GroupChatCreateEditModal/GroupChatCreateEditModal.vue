<script setup lang="ts">
	...
	const { openModalChatData, isGroupChatCreateModalOpen, temporalStorageForGroupChat, temporalStorageForDeletedUsers, temporalStorageForAddedUsers } = storeToRefs(chatsStore);
	/**
	...
	const groupChatUsersTotal = computed(() => {
		...
		const totalUsers = isGroupChatCreateModalOpen.value ? temporalStorageForGroupChat.value?.users.length : temporalStorageForGroupChat.value?.users.length;
	...
	>
		...
		>
			...
			<div class="edit-group__group-users-data">
				...
				<div class="edit-group__group-users">
					<GroupChatUser
