<template>
	<div class="map">
		<MapTop
			...
			:draw-button-active="drawButtonActive"
			@toggle-draw="status => toggleDraw(status)"
		/>
		...
		<div class="map__wrapper">
			<YandexMap
				...
				:settings="{
					location: {
						center: aloneItem ?? [37.5477, 55.643393],
						zoom: 15,
					},
...
<script setup>
	...
	import { YandexMap, YandexMapControlButton, YandexMapControls, YandexMapDefaultFeaturesLayer, YandexMapDefaultSchemeLayer, YandexMapFeature, YandexMapMarker, YandexMapZoomControl } from "vue-yandex-maps";

	import { computed, ref, shallowRef } from "vue";
	import MapTop from "~/components/ui/AppInputs/Address/Map/MapTop/MapTop.vue";
	import MapMarker from "~/components/ui/AppIcons/MapMarker/MapMarker.vue";
	...
	const emit = defineEmits(["selectPoints"]);
	...
	const drawNestedMarkers = (bounds, context) => {
		...
		for (const marker of modifyMarkers.value) {
			...
			if (bounds[0][0] < +marker.coords[0] && +marker.coords[0] < bounds[1][0] && bounds[1][1] < +marker.coords[1] && +marker.coords[1] < bounds[0][1]) {
	...
	const toggleDraw = status => {
		...
		initDrawEventsCanvas(bounds).then(function (coordinates) {
