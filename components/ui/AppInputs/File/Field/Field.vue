<template>
	...
	>
		...
		>
			...
			<template #footer>
				<Upload
					...
					:button-title="props.item.buttonName"
					:is-multiple="props.isMultiple"
					:is-icon="props.isIcon"
					:button-icon="props.isIcon ? localImages[0] : null"
					@call-action="data => callAction(data)"
				/>
			</template>
		</draggable>
		...
		<div
			v-if="props.isOneFile && props.isReadOnly && localImages.length > 2"
...
<script setup>
	...
	const emit = defineEmits(["changeValue", "initEdit"]);
	...
	const setClasses = computed(() => {
		return [localImages.value.length === 1 && props.isReadOnly ? "form-item__value_empty" : "", props.isOneFile ? "file-container_one-file" : ""];
	});
