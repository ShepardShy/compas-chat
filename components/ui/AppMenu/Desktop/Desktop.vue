<template>
	...
	>
		<a
			:href="menu.length > 0 ? menu[0].link : '/'"
			class="menu__logo"
		>
			<IconLogo />
		</a>
		...
		<div class="menu__content">
			...
			>
				<summary class="menu__item">
					<span class="menu__item-subtitle"> Еще </span>
				...
				<nav class="menu__list">
					...
					<li
						...
						@click="() => detailsMenuRef.removeAttribute('open')"
					>
						Скрыть
					</li>
				</nav>
			...
			>
				...
				<template #content>
					<template v-if="settingsMenu.activeTab == null">
						<PopupOption
							v-for="tab in settingsMenu.tabs"
							:key="tab.title"
					...
					<template v-else>
						...
						<template v-else>
							<PopupOption
								v-for="option in settingsMenu.activeTab.tab == 'enabled' ? menu : menu.filter(p => p.enabled)"
								:key="option.id"
		...
		>
			<template #summary>
				<div class="menu__user menu-user menu__item">
					...
					<div class="menu-user__title">Денис Потемкин</div>
				</div>
			</template>
			<template #content>
				<PopupOption> Настройки </PopupOption>
				<PopupOption class="popup__option_red"> Выйти </PopupOption>
			</template>
		</AppPopup>
	</aside>
</template>
...
<script setup>
	...
	import IconLogo from "~/components/ui/AppIcons/Logo/Logo.vue";
	...
	const callAction = data => {
		...
		const changeValue = data => {
			const findedOption = menu.value.find(option => option.slug == data.key);
		...
		const dragEnd = event => {
			if (event.to.__draggable_component__.itemKey == "menu-hidden") {
				const findIndex = menuHidden.value.findIndex(item => item.id == event.target.__draggable_component__.context.element.id);
				menuHidden.value[findIndex].is_hidden = 1;
			} else {
				const findIndex = menuVisible.value.findIndex(item => item.id == event.target.__draggable_component__.context.element.id);
