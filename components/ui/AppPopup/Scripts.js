export default {
  ...
  hideDetails (element) {
    switch (element) {
      ...
      case 'All':
        const data = document.querySelectorAll('.popup')
  ...
  setDropdownPosition (element) {
    ...
    if (element != null) {
      setTimeout(() => {
        const dropdown = element.querySelector('.popup__content')
        const dropdownRect = dropdown.getBoundingClientRect()
        if (scrollBlock != null) {
          const scrollBlockRect = scrollBlock.getBoundingClientRect()
          if (
        ...
        } else if (dropdownRect.top + dropdownRect.height > window.innerHeight) {
          element.classList.add('popup_up')
        } else {
          element.classList.remove('popup_up')
        }
