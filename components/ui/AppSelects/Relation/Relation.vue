	...
	>
		...
		<div class="relation__list">
			<RelationItem
				...
				@call-action="data => callAction(data, index)"
		...
		<div class="relation__actions">
			<ButtonText
				v-show="values.length >= 5"
				@click="() => callAction({ action: 'showAll', value: true })"
			>
				Всего {{ values.length }}, посмотреть все
			</ButtonText>
			<ButtonText
				v-if="props.isMultiple"
				@click="() => callAction({ action: 'changeValue', value: { value: null, isNew: true } })"
			>
				+ Добавить
			</ButtonText>
		</div>
...
<script setup>
	...
	const emit = defineEmits(["showAll", "openLink", "changeValue", "createOption", "clickOutside"]);
	...
	const callAction = (data, index) => {
		...
		// Получение опций
		const getOptions = () => {
			const filteredLocalOptions = props.item.value != null && props.item.value.localOptions != null ? props.item.value.localOptions.filter(p => ![null, undefined].includes(p) && typeof p === "object") : [];
			const options = JSON.parse(JSON.stringify(props.item.options.concat(filteredLocalOptions)));
