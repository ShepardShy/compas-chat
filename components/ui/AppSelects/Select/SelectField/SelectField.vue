<template>
	<div
		v-if="props.isMultiple && props.isReadOnly"
		class="form-item__value form-item__value_multiply"
		:class="activeOptions == null || activeOptions.length == 0 ? 'form-item__value_empty' : ''"
	>
		<div
			v-for="tab in activeOptions"
			:key="tab.label"
			class="select__active-option-tab"
		>
			{{ tab.label }}
		</div>
	</div>
	...
	>
		<template #summary>
			...
			>
				<div
					v-show="!props.isReadOnly && (props.isMultiple || [null, undefined].includes(search) || search == '')"
					class="select__active-option active-option"
				>
					<template v-if="props.isMultiple && activeOptions != null">
						...
						>
							...
							<IconDelete @click="event => callAction({ action: 'changeValue', value: tab.value, event: event })" />
						</div>
		...
		<template #content>
			<PopupOption
				v-show="props.isHaveNullOption & !props.isMultiple || options.length == 0"
			...
			<PopupOption
				...
				:class="{
					popup__option_active: option.value == activeOptions.value || multiplyValues.includes(option.value),
...
<script setup>
	...
	const callAction = async data => {
		...
		const getOptions = async () => {
			...
			const localOptions = props.item.options == null ? [] : props.item.options.filter(p => p != null && typeof p === "object" && !Array.isArray(p) && !isEmpty(p));
		...
		const setActiveOptions = async value => {
			...
			if (props.isMultiple) {
				...
				for (const item of multiplyValues.value) {
