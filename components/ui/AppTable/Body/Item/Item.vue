<template>
	<td
		...
		:class="[props.item.fixed ? 'table__item_fixed' : '', !props.item.enabled ? 'table__item_hidden' : '', props.item.isUpdated ? 'table__item_updated' : '']"
	...
	>
		<div class="table-item__content">
			<AppCheckbox
				...
				@change-value="data => changeValue(props.row.id, data)"
				@click-outside="() => callAction({ action: 'openPopup', value: false })"
			/>
			<AppRelation
				...
				:is-can-create="true"
				:is-multiple="Boolean(props.item.is_plural)"
				:is-read-only="Boolean(props.item.read_only || !props.row.isEdit)"
				@click="() => callAction({ action: 'openPopup', value: true })"
				@change-value="data => changeValue(props.row.id, data)"
				@open-link="data => callAction({ action: 'openLink', value: { id: data.id, slug: props.item.related_table } })"
				@show-all="() => callAction({ action: 'openLink', value: { id: props.row.id, slug: props.slug, tab: props.item.related_table } })"
				@create-option="() => emit('callAction', { action: 'createOption', value: props.item.related_table })"
				@click-outside="() => callAction({ action: 'openPopup', value: false })"
			/>
			<AppTextarea
				...
				:is-link="Boolean(props.item.is_external_link)"
				:is-read-only="Boolean(props.item.read_only || !props.row.isEdit)"
				@change-value="data => changeValue(props.row.id, data)"
				@click-outside="() => callAction({ action: 'openPopup', value: false })"
			/>
