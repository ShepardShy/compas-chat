<template>
	<div class="table-template__footer table-footer">
		<div class="table-footer__count-selected"><span class="table-footer__count-selected-title">Выбрано: </span> {{ bodyData.filter(p => p.isChoose).length }}</div>

		<AppPagination
			:total-pages="footerData.pages"
			:active-page="footerData.activePage"
			class="table-footer__pagination"
			@call-action="data => callAction({ action: 'changePage', value: data.value })"
		/>
		...
		<div class="table-footer__visible-elems">
			<AppSelect
				...
				@change-value="data => callAction({ action: 'changeVisibleElems', value: data.value })"
			/>
		</div>
	</div>
</template>
