...
export default {
  ...
  resizableGrid (table, fields) {
    ...
    const setListeners = (div) => {
      ...
      div.addEventListener('mousedown', function (e) {
        ...
        const padding = paddingDiff(info.curCol)
      ...
      div.addEventListener('mouseover', function (e) {
        const hoverElem = e.target.closest('.table__item_fixed')
        if (hoverElem != null) {
      ...
      document.addEventListener('mousemove', (e) => {
        const resizeCell = (e, info) => {
          if (info.curCol) {
            const diffX = info.curColWidth + e.pageX - info.pageX
            if ((diffX) >= 40) {
      ...
      document.addEventListener('mouseup', (e) => {
        ...
        function clearCell () {
          if (info.curCol != undefined) {
            const borderBlock = info.curCol.querySelector('.table-item__border')
    ...
    const sectionBody = table.parentNode
    const row = tableHeader.querySelector('tr')
    ...
    for (let i = 0; i < cols.length; i++) {
      const div = cols[i].querySelector('.table-item__border')
      setListeners(div)
    }
  },
  ...
  setStickyClass (table) {
    ...
    const scrolledArea = table.parentNode.scrollLeft
...
const onMouseMoveThrottle = _.throttle(async function (table, tableHeader, sectionBody, cell, width, info) {
  ...
  const data = table.querySelectorAll('tbody .table__row')
  let rowFields = []
  data.forEach((row) => {
...
const setVisibleTitle = (cell) => {
  const span = cell.querySelector('span') ?? cell.querySelector('.form-item__title')
...
const setCellsWidthDefference = (tableHeader, sectionBody) => {
  const changingCell = tableHeader.querySelector('th.changeWidth')
  const cells = tableHeader.querySelectorAll('th:not(.changeWidth)')
  let widthData = null
  ...
  const getProportionalСells = (cells, tableHeader, sectionBody) => {
    const widthData = []
    for (const cell of cells) {
      const part = getWidthPercent(cell)
      const width = getPartWidth(part.toFixed(2))
